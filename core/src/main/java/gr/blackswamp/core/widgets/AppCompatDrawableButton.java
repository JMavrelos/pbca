...
public class AppCompatDrawableButton extends AppCompatButton {
    int _tint = Color.TRANSPARENT;

    public AppCompatDrawableButton(Context context) {
        super(context);
    }

    public AppCompatDrawableButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public AppCompatDrawableButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs) {
        if (attrs == null)
            return;
        TypedArray attributeArray = getContext().obtainStyledAttributes(attrs, R.styleable.AppCompatDrawableButton);
        Drawable drawable_start = null;
        Drawable drawable_end = null;
        Drawable drawable_bottom = null;
        Drawable drawable_top = null;
        _tint = attributeArray.getColor(R.styleable.AppCompatDrawableButton_drawableTintCompat, Color.TRANSPARENT);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            drawable_start = attributeArray.getDrawable(R.styleable.AppCompatDrawableButton_drawableStartCompat);
            drawable_end = attributeArray.getDrawable(R.styleable.AppCompatDrawableButton_drawableEndCompat);
            drawable_bottom = attributeArray.getDrawable(R.styleable.AppCompatDrawableButton_drawableBottomCompat);
            drawable_top = attributeArray.getDrawable(R.styleable.AppCompatDrawableButton_drawableTopCompat);
        } else {
            int drawable_start_id = attributeArray.getResourceId(R.styleable.AppCompatDrawableButton_drawableStartCompat, -1);
            int drawable_end_id = attributeArray.getResourceId(R.styleable.AppCompatDrawableButton_drawableEndCompat, -1);
            int drawable_bottom_id = attributeArray.getResourceId(R.styleable.AppCompatDrawableButton_drawableBottomCompat, -1);
            int drawable_top_id = attributeArray.getResourceId(R.styleable.AppCompatDrawableButton_drawableTopCompat, -1);
